...
Use Go 1.24 or newer; a currently supported Go release is recommended.
...
vigiles diff --base-ref main package-lock.json
```

`--base-ref` compares the current file to the same path at a Git ref. It is a
reporting convenience and cannot be combined with `--strict`; use `gate` for
committed changes or MCP for working-tree checks with a trusted baseline.
...
- [x] Dependency diffs against a Git reference
- [x] Supported uv.lock and pnpm-lock.yaml dependency comparisons
